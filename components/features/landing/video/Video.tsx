...
const sampleMediaContent: MediaContentCollection = {
  video: {
    src: 'https://www.youtube.com/watch?v=V1RPi2MYptM',
    poster:
...
export const VideoExpansionTextBlend = () => {
  ...
  const [videoSrc, setVideoSrc] = useState<string>(currentMedia.src);
  ...
  useEffect(() => {
    ...
    async function fetchVideo() {
      try {
        const res = await fetch('/api/settings?key=videoSrc');
        if (!res.ok) return;
        const json = await res.json();
        if (!cancelled && typeof json.value === 'string' && json.value.trim() !== '') {
