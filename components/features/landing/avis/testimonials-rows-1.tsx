          ...
          ...new Array(2).fill(0).map((_, index) => (
            <React.Fragment key={index}>
              {props.testimonials.map(({ text, image, name, role }, i) => (
                <div className="p-10 shadow-lg shadow-primary/10 min-w-[300px] w-[300px] border border-foreground/10 rounded-3xl bg-white/5 text-foreground flex-shrink-0" key={i}>
                  <div>{text}</div>
