...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({ mediaItems}) => {
    ...
    const [items, setItems] = useState<MediaItemType[] | null>(null);
    const [isDragging, setIsDragging] = useState(false);
    const listItems = items ?? mediaItems;

    return (
        <div className="container w-full mx-auto overflow-hidden overscroll-none">
            <AnimatePresence mode="wait">
                {selectedItem ? (
                    <GalleryModal
                    ...
                    >
                        {listItems.map((item, index) => (
                            <motion.div
                                ...
                                className={`relative overflow-hidden rounded-xl cursor-move group ${item.span}`}
                                ...
                                onDragEnd={(e, info) => {
                                    ...
                                    if (Math.abs(moveDistance) > 50) {
                                        ...
                                        const targetIndex = moveDistance > 0 ?
                                            Math.min(index + 1, listItems.length - 1) :
                                            Math.max(index - 1, 0);
